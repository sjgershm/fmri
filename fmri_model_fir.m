function SPM = fmri_model(EXPT,model,submat)
    ...
    for subj = submat;
        ...
        SPM.xBF.dt = 10; %length of the post-stimulus epoch (in seconds)
        SPM.xBF.UNITS   = 'secs';     % time units ('scans', 'secs')
        SPM.xBF.name    = 'finite impulse response'; % basis function type
		SPM.xBF.order = 10; %number of post-onset time points to model
        SPM.xBF.factor = [];
        SPM.xBF.Volterra = 1;
